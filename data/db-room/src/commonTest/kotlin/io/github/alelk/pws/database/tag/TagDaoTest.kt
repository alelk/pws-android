...
class TagDaoTest : FeatureSpec({
  ...
  feature("tag dao crud") {
    ...
    scenario("insert a tag") {
      checkAll(10, Arb.Companion.tagEntity().removeEdgecases()) { tag ->
          db.tagDao().insert(tag)
      }
      db.tagDao().count() shouldBe 10
    }

    val tags = Arb.Companion.tagEntity().removeEdgecases().distinctBy { it.id }.take(10).toList()
    ...
    scenario("get tag by id") {
        checkAll(5, Arb.Companion.of(tags)) { tag ->
