...
class SongNumberDaoTest : FeatureSpec({
  ...
  feature("song number dao test") {
    ...
    val books = Arb.Companion.bookEntity().removeEdgecases().take(5).toList().distinctBy { it.id }
    val bookIds = books.map { it.id }
    scenario("insert books") {
      db.bookDao().insert(books)
    }

    val songs = Arb.Companion.songEntity().take(100).toList().distinctBy { it.id }
    ...
    val songNumbers =
      Arb.Companion
        .songNumberEntity(bookId = Arb.Companion.of(bookIds), songId = Arb.Companion.of(songIds)).take(500)
