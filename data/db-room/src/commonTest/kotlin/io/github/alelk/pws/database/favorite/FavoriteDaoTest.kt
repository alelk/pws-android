      ...
      db.withSongEntities(countSongs = 500) {
        db.withSongNumberEntities(countSongNumbers = 600) { songNumbers ->
          ...
          scenario("insert favorite") {
              checkAll(20,
                  Arb.Companion.favoriteEntity(songNumberId = Arb.Companion.of(songNumbers.map { it.id }))
                      .distinctBy { it.songNumberId }.removeEdgecases()
              ) { favorite ->
          ...
          scenario("toggle favorite") {
            checkAll(10, Arb.Companion.of(songNumbers).distinct()) { songNumber ->
