...
`TelemetryConsentStore` умеет три состояния, и это важно для приватности, а не только для UX:

| Состояние | Когда | Что отправляется |
|---|---|---|
| **pending** | ключ в prefs отсутствует — первый запуск | **ничего** (SDK активирован, `dataSending=false`) |
| enabled | пользователь оставил отметку / включил тумблер | всё по словарю событий |
| disabled | пользователь снял отметку / выключил тумблер | ничего |

Pending разрешается ровно один раз, в `MainActivity`:

- **показан онбординг** → пользователь видит блок раскрытия с чекбоксом (по умолчанию отмечен) и
  ссылкой на политику; ответ фиксируется при выходе с экрана, **до** первого события;
- **онбординг не показан** (обновление поверх старой версии, сборка с вшитым сборником) → берётся
  `defaultConsent` (`!BuildConfig.DEBUG`), раскрытие для таких пользователей — Настройки →
  Приватность и карточка магазина.

Отсюда следствие для отладки: на чистой установке в логе будет `dataSending=false` до тех пор, пока
не пройден онбординг — это норма, а не поломка.
...
2. В приложении: **Настройки → Приватность** → включить тумблер «Отправлять отчёты о сбоях и
   статистику использования» (в debug он выключен по умолчанию, чтобы не засорять статистику
   dev-запусками, — в том числе чекбокс на онбординге).
...
| `AppMetrica activated (… dataSending=false) — nothing will be sent…` | ключ есть, но согласия нет: либо тумблер выключен, либо это чистая установка и онбординг ещё не пройден | пройти онбординг с отмеченным чекбоксом или Настройки → Приватность → включить тумблер |
