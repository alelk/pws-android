...
class SongActivity : AppCompatThemedActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    val songNumberId = intent.getStringExtra(KEY_SONG_NUMBER_ID)?.takeIf { it.isNotBlank() }?.let { SongNumberId.Companion.parse(it) }
    Timber.Forest.d("song activity created: song number id = $songNumberId")
    ...
    lifecycleScope.launch {
      repeatOnLifecycle(Lifecycle.State.STARTED) {
        songViewModel.allBookNumbers.collect { allBookNumbers ->
          ...
          Timber.Forest.d("book ${songViewModel.song.value?.book?.id} has ${songNumberIds?.size} song numbers")
          if (songNumberIds != null) {
            (songTextPager.adapter as? SongTextFragmentPagerAdapter)
              ?.let { it.allSongNumberIds = songNumberIds }
              ?: run {
                songTextPager.adapter =
                    SongTextFragmentPagerAdapter(this@SongActivity, songNumberIds)
                songTextPager.currentItem = songNumberIds.indexOf(songNumberId)
                songTextPager.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
  ...
  override fun onOptionsItemSelected(item: MenuItem): Boolean =
    when (item.itemId) {
      R.id.menu_jump -> {
        ...
        if (bookId != null)
          JumpToSongByNumberDialogFragment.Companion.newInstance(bookId).show(supportFragmentManager, JumpToSongByNumberDialogFragment::class.java.simpleName)
        true
      }
