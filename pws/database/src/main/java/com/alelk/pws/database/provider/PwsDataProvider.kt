...
class PwsDataProvider : ContentProvider(), PwsDataProviderContract {
    ...
    override fun query(
        ...
        when (URI_MATCHER.match(uri)) {
            Psalms.URI_MATCH -> cursor = mDatabase!!.query(PwsPsalmTable.TABLE_PSALMS, projection, selection, selectionArgs, null, null, sortOrder)
            Psalms.URI_MATCH_ID -> {}
            Psalms.PsalmNumbers.URI_MATCH -> {}
            Favorites.URI_MATCH -> cursor = queryFavorites(projection, selection, selectionArgs, null, null)
            Favorites.URI_MATCH_ID -> cursor = queryFavorite(uri.lastPathSegment.toLong())
            History.URI_MATCH -> cursor = queryHistory(projection, selection, selectionArgs, null, uri.getQueryParameter(QUERY_PARAMETER_LIMIT))
            History.Last.URI_MATCH -> cursor = queryHistory(projection, null, null, History.Last.SORT_ORDER, History.Last.LIMIT)
            Psalms.Suggestions.URI_MATCH_NUMBER -> cursor = querySuggestionsPsalmNumber(uri.lastPathSegment, uri.getQueryParameter(SearchManager.SUGGEST_PARAMETER_LIMIT))
            Psalms.Suggestions.URI_MATCH_NAME ->
                if (uri.lastPathSegment != null) {
                    val searchText = uri.lastPathSegment
                    cursor = searchText?.let { querySuggestionsPsalmName(it, uri.getQueryParameter(SearchManager.SUGGEST_PARAMETER_LIMIT)) }
                    if (cursor == null || cursor.count < 1) cursor = querySuggestionsPsalmText(searchText, uri.getQueryParameter(SearchManager.SUGGEST_PARAMETER_LIMIT))
                }
            Psalms.Search.URI_MATCH -> {
                if (selectionArgs?.isNotEmpty() == true) {
                    try {
                        ...
                        var text = if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) selectionArgs[0].lowercase(Locale.getDefault()) else selectionArgs[0]
        ...
        Log.v(LOG_TAG, METHOD_NAME + ": Query for uri='" + uri.toString() + "'. Results: " + cursor.count)
        ...
        when (URI_MATCHER.match(uri)) {
            Psalms.URI_MATCH -> {
                ...
                if (id == -1L) {
                    ...
                    Log.w(LOG_TAG, METHOD_NAME + ": Error inserting into '" + PwsPsalmTable.TABLE_PSALMS + "' table. Uri='" + uri + "'")
                    return null
                }
            ...
            Favorites.URI_MATCH -> {
                ...
                if (id == -1L) {
                    // TODO: 24.02.2016 throw exception
                    Log.w(LOG_TAG, METHOD_NAME + ": Error inserting into '" + PwsFavoritesTable.TABLE_FAVORITES + "' table. Uri='" + uri + "'")
                    return null
                }
                itemUri = ContentUris.withAppendedId(uri, id)
            ...
            History.URI_MATCH -> {
                ...
                if (id == -1L) {
                    // TODO: 24.02.2016 throw exception
                    Log.w(LOG_TAG, METHOD_NAME + ": Error inserting into '" + PwsHistoryTable.TABLE_HISTORY + "' table. Uri='" + uri + "'")
                    return null
                }
                itemUri = ContentUris.withAppendedId(uri, id)
                mContext!!.contentResolver.notifyChange(uri, null)
    ...
    private fun queryFavorites(projection: Array<String>?,
                               selection: String?,
                               selectionArgs: Array<String>?,
                               orderBy: String?,
                               limit: String?): Cursor? {
        ...
        val cursor = mDatabase!!.query(Favorites.TABLES,
    ...
    private fun queryHistory(projection: Array<String>?,
                             selection: String?,
                             selectionArgs: Array<String>?,
                             orderBy: String?,
                             limit: String?): Cursor {
    ...
    private fun queryLastFavorite(projection: Array<String>?): Cursor? {
        ...
        Log.v(LOG_TAG, METHOD_NAME + ": projection=" + Arrays.toString(projection) + " results: " + (cursor?.count
                ?: "cursor=null"))
        return cursor
    }

    private fun querySuggestionsPsalmNumber(psalmNumber: String,
                                            limit: String?): Cursor {
    ...
    private fun querySuggestionsPsalmName(searchName: String, limit: String?): Cursor {
        return mDatabase!!.query(Psalms.Suggestions.SG_NAME_TABLES,
                ...
                Psalms.Suggestions.getSgNameSelection(searchName.lowercase(Locale.getDefault())), null, Psalms.Suggestions.SG_NAME_GROUPBY,
                null, null, limit)
    }
    ...
    private fun querySuggestionsPsalmText(searchName: String, limit: String?): Cursor {
        return mDatabase!!.query(Psalms.Suggestions.SG_TXT_TABLES,
                ...
                Psalms.Suggestions.getSgTextSelection(searchName.lowercase(Locale.getDefault())), null, Psalms.Suggestions.SG_TXT_GROUPBY,
                null, null, limit)
    }

    private fun querySearchPsalmNumber(psalmNumber: Int,
                                       limit: String?): Cursor {
    ...
    private fun querySearchPsalmText(searchText: String,
                                     limit: String?): Cursor? {
        val METHOD_NAME = "querySearchPsalmText"
        val cursor = mDatabase!!.query(Psalms.Search.S_TXT_TABLES,
                Psalms.Search.S_TXT_PROJECTION, Psalms.Search.getSTxtSelection(searchText), null,
                null, null,
                Psalms.Search.S_TXT_ORDER_BY, limit)
        Log.d(LOG_TAG, METHOD_NAME + ": searchQuery='" + searchText + "' results:" + (cursor?.count
                ?: 0))
        return cursor
    }

    private fun queryPsalmNumberPsalm(psalmNumberId: Long,
                                      projection: Array<String>?,
                                      selection: String?,
    ...
    private fun queryPsalmNumberBookPsalmNumbers(psalmNumberId: Long, projection: Array<String>?): Cursor? {
        return queryPsalmNumberBookPsalmNumber(psalmNumberId, projection, null, null)
    }

    private fun queryPsalmNumberBookPsalmNumber(psalmNumberId: Long,
                                                projection: Array<String>?,
                                                selection: String?,
                                                selectionArgs: Array<String?>?): Cursor? {
        var projection = projection
        ...
        val rawQuery = SQLiteQueryBuilder.buildQueryString(false,
    ...
    private fun queryPsalmNumberBookPsalmNumberInfo(psalmNumberId: Long, projection: Array<String>?): Cursor? {
        var projection = projection
    ...
    private fun queryBookStatistic(projection: Array<String?>?,
                                   selection: String?,
                                   selectionArgs: Array<String>?,
    ...
    private fun updateBookStatistic(values: ContentValues, bookEdition: String): Int {
        val rawSelection = SQLiteQueryBuilder.buildQueryString(false,
