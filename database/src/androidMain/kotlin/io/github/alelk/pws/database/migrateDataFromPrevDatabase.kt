...
internal suspend fun migrateDataFromPrevDatabase(context: Context, currentDatabase: PwsDatabase) =
  DATABASE_PREV_NAMES.forEach { dbName ->
    runCatching {
      val dbFile = context.getDatabasePath(dbName)
      val prevDb = dbFile?.let(::openReadOnlyDatabase)
      if (prevDb != null) {
        Timber.i("found previous database ${prevDb.path} (v${prevDb.version}), migrate user data to $DATABASE_NAME...")
        val dataProvider = PwsDb1xDataProvider(prevDb)
        ...
        if (favorites.isSuccess && history.isSuccess && editedSongs.isSuccess && tags.isSuccess) {
