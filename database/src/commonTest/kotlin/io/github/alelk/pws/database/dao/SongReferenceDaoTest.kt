...
class SongReferenceDaoTest : FeatureSpec({
  ...
  feature("song song reference dao crud test") {
    ...
    scenario("insert songs") {
      val songsToInsert = Arb.songEntity().removeEdgecases().take(50).distinctBy { it.id }.toList()
    ...
    val songRefArb =
      Arb.songSongReferenceEntity(
        songId = Arb.of(songIds).removeEdgecases(),
        refSongId = Arb.of(songIds).removeEdgecases()
