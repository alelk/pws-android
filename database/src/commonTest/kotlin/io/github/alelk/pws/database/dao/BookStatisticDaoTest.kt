...
class BookStatisticDaoTest : FeatureSpec({
  ...
  feature("book statistic dao crud test") {
    ...
    val books = Arb.bookEntity().take(100).toList()

    scenario("insert book") {
      db.bookDao().insert(books)
    }

    val bookStatistics = Arb.bookStatisticEntity(id = Arb.of(books.map { it.id }).removeEdgecases()).distinctBy { it.id }.take(50).toList()
