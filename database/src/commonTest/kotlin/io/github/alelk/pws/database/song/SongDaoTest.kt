...
class SongDaoTest : FeatureSpec({
  ...
  feature("song dao crud test") {
    scenario("insert songs one by one and get by id") {
        checkAll(10, Arb.Companion.songEntity().removeEdgecases()) { song ->
    ...
    scenario("delete song") {
        checkAll(10, Arb.Companion.songEntity()) { song ->
            db.songDao().insert(song)
  ...
  feature("insert/get/delete song collection") {
    val songs = Arb.Companion.songEntity().removeEdgecases().distinctBy { it.id }.take(20).toList().distinctBy { it.id }
    ...
    scenario("delete songs collection") {
      val extraSong = Arb.Companion.songEntity().next()
